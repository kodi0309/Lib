...
<body>	
	...
	<div id="tresc">	
		<table border="1" bordercolor="#FFFFFF" class="dane" cellpadding="5">
			<tr><td> LP. </td>	<td> KLASA </td>	<td> EDYTUJ </td></tr>

				<?PHP
				include 'db_conf.php';
				$baza = mysqli_connect($db_server,$db_user, $db_pass, $db_name) or die ('error'); 
				$zapytanie="SELECT klasy.idklasa, klasy.klasa FROM klasy ORDER BY klasa";
				$result = $baza->query($zapytanie) or die ('bledne zapytanie');
				$lp=0;
				while ($wiersz = $result->fetch_assoc())
				 	{ 
				 	$lp=$lp+1;
				   	echo "<tr><td>".$lp ."</td><td>". $wiersz['klasa']."</td>";
				   	echo "<td> <a href='edytujkl.php?id=".$wiersz['idklasa']."'>'<img src='images/edit.png'>'</a> </td>";
				   	echo "</tr>\n";
				 	};
				 $baza->close();
				?>
